...
(function(){{
  ...
  function calcScale(){{
    ...
    return iw>60?Math.min(1.55,(iw-28)/595):1.2;
  }}
        ...
        with c_mail2:
            ...
            if sel_emails:
                ...
                for _idx, selected_label in enumerate(sel_emails):
                    ...
                    # Siempre expander contraído — el usuario elige cuál abrir
                    with st.expander(f"✉️ {info_sel['empresa']}", expanded=False):
                        # 1) Portada del email (primero)
                        st.markdown("**📧 Portada del email**")
                        components.html(cover_html, height=540, scrolling=True)

                        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

                        # 2) PDF adjunto (segundo)
                        st.markdown("**📄 PDF adjunto**")
                        if pdf_preview_b64:
                            components.html(
                            ...
                            st.warning("⚠️ Vista previa del PDF no disponible para este cliente.")

            else:
                st.info("Selecciona un cliente para ver la vista previa.")

        # ── Acción de Envío (full-width, fuera de columnas) ───────────────
        if sel_emails:
            st.markdown("---")
            # ── Acción de Envío ──────────────────────────────────────────
